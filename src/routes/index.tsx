...
const sendMessage = createForm(
  server(async function (form: FormData) {
    ...
    if (Math.random() > 0.75) {
      throw new FormError(
        "There was an error adding the messages, please try again"
      );
    }
  ...
  return (
    <li class="flex flex-row space-y-2">
      <div class="text-lg inline-flex flex-row items-center space-x-2">
        ...
        <span>
          {props.submission.variables.formData.get("message") as string}
        </span>
          ...
          {(error) => (
            <>
              ...
              <sendMessage.Form key={props.submission.key}>
                ...
                <button
                  type="submit"
                  class="bg-red-100 rounded-sm px-2 py-0 text-red-700"
                >
                  Retry
                </button>
              </sendMessage.Form>
...
const removeMessage = createForm(
  server(async (form: FormData) => {
    ...
    if (Math.random() > 0.75) {
      throw new FormError(
        "There was an error removing the player, please try again later"
      );
    }
...
function MessageItem(props: { item: Message & { user: User } }) {
  return (
    <div
      class="py-1"
      style={{
        color: removeMessage.submissions()[props.item.id]?.error
          ? "red"
          : "black",
      }}
    >
  ...
  function MessageBox() {
    ...
    return (
      ...
      >
        ...
        />
        <button
          class="focus:bg-gray-100 bg-gray-200 rounded-md px-2"
          type="submit"
          id="sends"
        >
          Send
